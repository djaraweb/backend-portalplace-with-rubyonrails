...
RSpec.describe "Posts with authentication", type: :request do
  ...
  describe "2. POST /posts" do
    # con auth -> crear
    context "with valid auth" do
      before { post "/posts", params: create_params, headers: auth_headers }
      it "should create a post" do
        subject {payload}
        # payload = JSON.parse(response.body).with_indifferent_access # ok una forma
        #expect(payload).to include(:id, :title, :content, :published, :author) # validar no acepta autor
        expect(payload).to include(:id, :title, :content, :published) 
      end

      context "payload" do
        subject { payload }
        # it { is_expected.to include(:id, :title, :content, :published, :author) } # validar no acepta autor     
